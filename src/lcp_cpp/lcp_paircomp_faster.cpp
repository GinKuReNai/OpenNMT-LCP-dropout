/**
 * @file lcp_paircomp_faster.cpp
 * @author Akito Koga (koga.akito893@mail.kyutech.jp)
 * @brief PairCompression-based LCP-dropout (Hash Table Used)
 * @version 0.1
 * @date 2023-02-14
 *
 ...
 * @return tuple<vector<string>, unordered_map<string, list<int>>>
 */
static tuple<vector<string>, unordered_map<string, list<int>>> txt2voc(string_view text)
{
    // テキストの先頭からサブワードを順に保存する配列
    vector<string> vocab;
    // サブワードのインデックスをそれぞれ保存する連想配列
    unordered_map<string, list<int>> vocabIndexesMap;
    // 文字数カウンタ
    int wordNum = 0;
    ...
    for (char c : text)
    {
        ...
        {
            vocab.push_back("_");
            insertIndex(vocabIndexesMap, "_", wordNum);
            ++wordNum;
        }
        else if (hashWord == hashNewline)
        {
            vocab.push_back("<EOS>");
            insertIndex(vocabIndexesMap, "<EOS>", wordNum);
            ++wordNum;
        ...
        else
        {
            ...
            insertIndex(vocabIndexesMap, word, wordNum);
            ++wordNum;
        }
    }

    return {vocab, vocabIndexesMap};
}
 ...
 *
 * @param vocab vector<string>
 * @param vocabIndexesMap unordered_map<string, list<int>>
 * @param alphabets <string>
 * @param pref size_t
 * @param prefAddress string
 *
 * @return tuple<vector<int>, unordered_map<pair<string, string>, vector<int>>> 
 */
static tuple<vector<int>, unordered_map<pair<string, string>, vector<int>>> pairCompression(
    vector<string> const &vocab,
    unordered_map<string, list<int>> const &vocabIndexesMap,
    set<string> &alphabets,
    size_t const &pref,
    string const &prefAddress)
{
    list<int> indexes;                                                 /* 対象サブワードのインデックスのリスト(vocabIndexesMapから取り出す) */
    unordered_map<pair<string, string>, vector<int>> pair01IndexesMap; /* 01となるペアとインデックスを保存 */
    vector<int> zerooneList;
    string startWord;
    string pointWord;
    ...
    // 開始文字のインデックスを取得
    auto it = vocabIndexesMap.find(startWord);
    indexes = it->second;
    ...
    while (alphabets.size())
    {
        ...
        // 基準文字のインデックスを取得
        auto it = vocabIndexesMap.find(pointWord);
        indexes = it->second;

        // 基準文字の右隣の文字x∈∑となるパターンが存在するかどうか
        for (int index : indexes)
        {
            if (find(alphabets.begin(), alphabets.end(), vocab[index + 1]) != alphabets.end() && index != (vocabSize - 1))
            {
                targetWord = vocab[index + 1];
                isFindPattern = true;
        ...
        {
            // 対象文字のインデックスを取得
            auto it = vocabIndexesMap.find(targetWord);
            indexes = it->second;
            // x? or ?x となるような2-gramを走査
            for (int index : indexes)
            {
                // x? かつ 右隣の符号が0の場合
                if (index != vocabSize - 1 && zerooneList[index + 1] == 0)
                {
                    zeroSigmaCount++;
                }
                // x? かつ 右隣の符号が1の場合
                if (index != vocabSize - 1 && zerooneList[index + 1] == 1)
                {
                    oneSigmaCount++;
                }
                // ?x かつ 左隣の符号が0の場合
                if (zerooneList[index - 1] == 0)
                {
                    zeroSigmaCount++;
                }
                // ?x かつ 左隣の符号が1の場合
                if (zerooneList[index - 1] == 1)
                {
                    oneSigmaCount++;
                }
            }
            ...
            if (zeroSigmaCount >= oneSigmaCount)
            {
                for (int index : indexes)
                {
                    ...
                    // ?xのペアが01の場合, ペアとインデックスを保存
                    if (zerooneList[index - 1] == 0)
                    {
                        pair<string, string> pair = {vocab[index - 1], vocab[index]};
                        // 01のペアが既出の場合
                        if (pair01IndexesMap.find(pair) != pair01IndexesMap.end())
                        {
                            pair01IndexesMap[pair].push_back(index - 1);
                        }
                        // 01のペアが未出現の場合
                        else
                        {
                            pair01IndexesMap[pair] = {index - 1};
                        }
                    }
                }
            }
            else
            {
                for (int index : indexes)
                {
                    ...
                    // x?のペアが01の場合, ペアとインデックスを保存
                    if (index != vocabSize - 1 && zerooneList[index + 1] == 1)
                    {
                        pair<string, string> pair = {vocab[index], vocab[index + 1]};
                        // 01のペアが既出の場合
                        if (pair01IndexesMap.find(pair) != pair01IndexesMap.end())
                        {
                            pair01IndexesMap[pair].push_back(index);
                        }
                        // 01のペアが未出現の場合
                        else
                        {
                            pair01IndexesMap[pair] = {index};
                        }
                    }
                }
            }
    ...
    return {zerooneList, pair01IndexesMap};
}
 ...
 *
 * @param vocab vector<string>
 * @param pair01IndexesMap unordered_map<pair<string, string>, vector<int>>
...
static tuple<vector<pair<string, string>>, vector<int>> get01Pairs(
    vector<string> const &vocab,
    unordered_map<pair<string, string>, vector<int>> &pair01IndexesMap,
    ...
    int const topk)
{
    ...
    // 出現頻度上位k(%)に含まれるペアの数
    int mergeNum = static_cast<int>(pair01IndexesMap.size() * topk / 100);

    // pair01IndexesMapを出現頻度順にソート
    vector<pair<pair<string, string>, vector<int>>> orderedPair01IndexesMap = sortMapByDescOrder(pair01IndexesMap);

    // ソート済みpair01IndexesMapからペアとインデックスをそれぞれ分ける
    for (int i = 0; i < mergeNum; ++i)
    {
        // ペアを保存
        topkIncludePairs.push_back(orderedPair01IndexesMap[i].first);
        // インデックスを保存
        topkIncludeIndexes.insert(
            topkIncludeIndexes.end(),
            orderedPair01IndexesMap[i].second.begin(),
            orderedPair01IndexesMap[i].second.end());
    }
    ...
    double const dropoutRate)
{
    ...
    for (int index : topkIncludeIndexes)
    {
        ...
        {
            string newPair = vocab[index] + vocab[index + 1];
            vocab[index] = newPair;
            vocab[index + 1] = "///";
        }
    }
    ...
    double const dropoutRate)
{
    ...
    vector<string> vocab;
    unordered_map<string, list<int>> vocabIndexesMap;
    tie(vocab, vocabIndexesMap) = txt2voc(text);
    ...
    while (mergedPairCount <= vocSize)
    {
        ...
        {
            // 一度全消去してから, 新しく作り直す
            vocabIndexesMap.clear();
            for (int i = 0, vocabSize = vocab.size(); i < vocabSize; ++i)
            {
                insertIndex(vocabIndexesMap, vocab[i], i);
            }
        }
        ...
        if (prefList.empty())
        {
            tie(zerooneList, pair01IndexesMap) = pairCompression(vocab, vocabIndexesMap, alphabets, -1, prefAddress);
        }
        else
        {
            tie(zerooneList, pair01IndexesMap) = pairCompression(vocab, vocabIndexesMap, alphabets, prefList[lcpLoopCount], prefAddress);
        }

        // ラベル 01 を探索し出現回数を計測
        vector<pair<string, string>> topkIncludePairs;
        vector<int> topkIncludeIndexes;
        tie(topkIncludePairs, topkIncludeIndexes) = get01Pairs(vocab, pair01IndexesMap, zerooneList, topk);

        // 01 となるペアを結合
        mergeVocab(vocab, vocabIndexesMap, topkIncludeIndexes, dropoutRate);
